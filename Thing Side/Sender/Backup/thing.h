...
size_t msgbuf_length = 8; // Length of message buffer
key_t shm_key_sender = (key_t) 2000; // Key of sender shared memory used to verify server availability
...
void checkserver() {
    ...
    if ((shmid = shmget(shm_key_sender, 1, IPC_CREAT | 0444)) < 0) {
        perror("shmget");
        //printf("\n key: %x", key);
        printf("\n error1");
    }
