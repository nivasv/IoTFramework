...
main(int argc, char **argv) {
    ...
    //char serverURL[] = "COAP://10.17.27.180:5683/Data";
    char *serverURL = NULL; // COAP://localhost:5683"
    ...
    int shmid_server;
    char server_str [] = "servercheck";

    int queue_size = 0;
    long shm_key_sender = 3000;
    ...
    while ((opt = getopt(argc, argv, "l:q:s:")) != -1) {
        switch (opt) {
            case 'l':
    ...
    if (segment_len == 0 || queue_size == 0 || serverURL == NULL) {
    ...
    char input_seg[(int) segment_len];
    /*
    ...
    /*
     * Shared memory for sending the data to checker if server is not available
     */

    for (i = 0; i < queue_size; i++) {

        key_shm = (key_t) (shm_key_sender + i);
    ...
    /*
     * Shared memory for Server status check
     */

    if ((shmid_server = shmget(shm_key_server, 1, IPC_CREAT | 0666)) < 0) {
        perror("shmget in shmid_server ");
        //printf("\n key: %x", key);
        printf("\n error1");
        exit(1);
    }
    ...
    if ((serverstatus = shmat(shmid_server, NULL, 0)) == (char *) - 1) {
        perror("shmat - shmid_server ");
        printf("\nshmid_server: %d", shmid_server);
    }

    strcpy(serverstatus, "1"); // Initialize
    ...
    while (1) {
        ...
        if (msgrcv(msqid_sender, &rbuf, msgbuf_length, 1, 0) < 0) {
            ...
            //printf("mserverstatus - %c", rbuf.mserverstatus);

            if (rbuf.mserverstatus == '1') {
                if (!cmdline_input(server_str, &payload))
                    payload.length = 0;
                ...
                if (result < 0) { /* error */
                    ...
                    if (max_wait <= now) {
                        ...
                        if (rbuf.mserverstatus != '1') {
                             ...
                             * other process to read.
                             */
                            strcpy(shm, input_seg);
                            wbuf.mtype = 1;
                            wbuf.key = (key_t) (shm_key_sender + key_int);
